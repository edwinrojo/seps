<div>
    ...
    @foreach ($statuses as $status)
        ...
        @endphp
        <li class="mb-6 ms-6">
            <span class="absolute flex items-center justify-center w-6 h-6 {{ $icon_background }} rounded-full -start-3 ring-8 ring-white dark:ring-gray-900 {{ $icon_background_dark }}">
                  <x-dynamic-component :component="$filament_icon" class="w-4 h-4 text-custom-800 fi-color-{{ $status->status->getColor() }}"/>
            </span>

            <h3 class="flex items-center mb-1 text-base font-semibold text-custom-800 fi-color-{{ $status->status->getColor() }} dark:text-white">{{ $status->status->getLabel() }}
